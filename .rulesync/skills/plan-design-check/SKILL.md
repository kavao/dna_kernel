...
- 状態記号が `[ ]` または `[x]` であること
- チェック項目に説明があること
- ヘッダに `バージョン: MAJOR.MINOR` または `Version: MAJOR.MINOR` があること
- `## 変更履歴` または `## Change History` があり、日付・バージョン・変更内容の表と1件以上の履歴行があること
- ヘッダのバージョンと変更履歴の最終行のバージョンが一致すること
...
- 変更履歴の既存行が書き換えられていないことは、追記専用の運用とレビューで確認する。検査ツールは、版の形式・履歴表・最終版の一致を確認する。

---
targets: ["*"]
root: true
description: "自己発展型ルールガバナンスの概念正本"
---

# concepts — 自己発展型ルールガバナンスの概念正本

ここには、プロジェクト全体の判断基準となる「短い定義・禁止・完了条件」だけを置きます。
長い例・コマンド・背景説明は `docs/` へ逃がしてください。

## 自己発展型ルールガバナンス

**正本（Policy-as-Code）**を短い概念として固定し、**指示（Instruction-driven）**で運用を拡張しつつ、
**機械検証**と**追記ログ**で「完了」を拘束して、再現可能に発展する運用方式。

## 正本と副本

定義:
正本は、判断・編集・検証の基準になる一次情報。副本は、人間の確認・既存バッチ連携・表示用に使う派生情報。

必須:

- 正本を更新できる状態では、副本だけを直接直して完了扱いしない。
- 副本を直した場合は、対応する正本へ反映してから再生成・再エクスポートする。
- どちらが正本か迷う領域では、作業前に既存ルール・スキルの「正本」節を確認する。
- `.rulesync/` を変更したときは、`python tools/rulesync.py generate --check` を実行し、生成物が正本と一致することを確認してから完了とする。

代表的な正本と生成物の対応:

- `.rulesync/rules/`、`.rulesync/skills/` → 編集する正本
- `.claude/`、`.cursor/`、`.codex/`、`.agents/`、`.grok/`、`AGENTS.md`、`CLAUDE.md` 等 → rulesync の生成物。直接編集せず、正本を直してから再生成する。
- `docs/ja/` → docs の編集正本、`docs/en/` → 表示用の同期副本

**会話言語と書き込み正本の分離**: LLM とのチャット言語（ホーム config → `user-locale`）と、ファイルへの書き込み正本（`content-placement` + `rule-authoring` / `docs-writing`）は別軸。日本語で会話していても docs の編集正本は `docs/ja/`、表示用 EN 同期は `docs/en/` のまま。

## 判断が衝突した場合の優先順位

定義:
複数の指示・正本・スキル・docsが矛盾する場合に、どれを優先するかを固定する。

必須:

- 優先順位は「ユーザーの明示依頼 → プロジェクト固有の正本 → `concepts.md` → 該当スキル → docsの説明」の順とする。
- 既存の設定・記述と衝突する場合、確認なしに統合・上書きしない。差分と選択肢を提示し、ユーザーの判断を待つ。

## 完了扱い条件

定義:
「完了」は、成果物が正本または保存先に実在し、確認が済んだ状態だけを指す。

必須:

- 正本へ書き込む。
- 書き込み後、再読込または機械検証で確認する。
- 完了報告の直前に、査証ログへ追記する（手順: `skills/workspace-audit-log/SKILL.md`）。
- 確認後に、更新ファイルパス（保存先）を添えて完了報告する。

禁止:

- チャット・会話欄への貼り付けだけで「完了」としない。
- 保存先のファイル確認前に「生成完了」「執筆完了」と言わない。

## 計画書・設計書の進捗管理

定義:
計画書・設計書の進捗は、文書ごとの自由な状態表記ではなく、共通チェックボックスで管理する。

必須:

- 進捗セクション（日本語は `## 進捗`、英語は `## Progress`）を置く。
- 未完了（未着手、進行中、部分完了、確認待ち、保留を含む）は `- [ ]`、完了は `- [x]` で記載する。
- `## 実装フェーズ` などの詳細チェック項目も進捗の一部として扱い、実装済みは `[x]`、未実施・確認待ちは `[ ]` に更新する。
- `## 進捗` の概要と詳細項目に同じ作業がある場合は、両方の状態を一致させる。詳細項目を未更新のまま概要だけ `[x]` にしない。
- 完了・部分完了などの表だけを進捗の正本にしない。部分完了や確認待ちは `[ ]` のまま残作業を補足する。
- 計画書・設計書のヘッダに `バージョン: MAJOR.MINOR`（英語文書は `Version: MAJOR.MINOR`）を置き、`## 変更履歴`（英語は `## Change History`）を `## 進捗` の前に置く。
- `## 変更履歴` は `日付 / バージョン / 変更内容`（英語は `Date / Version / Change`）の表とし、変更時はヘッダの版を更新して履歴行を末尾へ追記する。既存履歴の削除・上書き・並べ替えは行わない。
- 作成・更新後は `skills/plan-design-check/SKILL.md` の機械検査を実行し、終了コード0を確認する。

禁止:

- `[~]`、`[-]`、`完了` / `部分完了` だけの表記を進捗マーカーとして使わない。

## 査証ログの原則

定義:
作業事実は追記専用のログファイルに記録し、過去の記録を改ざんしない。

必須:

- 作業後は `_workingspace/log/YYYYMM.md` に追記する（`tools/kernel/workspace_audit_log.py` を使う）。
- 既存行の削除・上書き・並べ替えは行わない。
- 完了したチェックリストの削除で「記録した」と見なさない。作業事実は必ずログへ。

## テストによるデグレード防止

定義:
コード（ツール・スクリプト・モジュール）の変更が既存の動作を壊していないことを、テストコードの実行で機械的に確認する。

必須:

- コードを新規作成・変更したときは、対応するテストを書く（既存テストがあれば更新する）。
- 変更後、完了とする前にテストを実行し、終了コード 0 を確認する。
- テストの実行はユーザーの指示を待たず、自己判断で行う。

禁止:

- テストを実行せずにコード変更を「完了」とみなさない。
- テストが失敗した状態のまま「動く」「問題ない」と報告しない。
- テストを黙って削除・コメントアウトして通過させない。

参照:

- 実行手順: `skills/code-testing/SKILL.md`

## ルール配置の原則

定義:
短い定義・禁止・完了条件はこのファイル（concepts.md）に置き、手順・例・説明は他ファイルへ分ける。

必須:

- 事故防止に関わる必須条件は、まずここへ短く置く。
- 同じ禁止・完了条件を別表現で複数箇所に書かない。
- 手順の詳細は対応するスキルへ、操作説明は `docs/` へ分離する。

参照:

- ルール作成作法: `rules/rule-authoring.md`
- パターン説明: `docs/ja/dna-kernel/self-evolving-governance.md`（編集正本）、`docs/en/dna-kernel/self-evolving-governance.md`（英訳）

## プロジェクト導入モード

定義:
新しいプロジェクトへ dna_kernel を持ち込むときは、既存構成を確認し、新規作成か既存注入かを判定してから進める。

必須:

- 既存の `README.md`, `docs/`, `tools/`, `.rulesync/`, `rulesync.jsonc` の有無を確認する。
- 既存プロジェクトではルート `README.md` を上書き・移動しない。
- dna_kernel の説明は `docs/ja/dna-kernel/`（編集正本）と `docs/en/dna-kernel/`（英訳）へ置く。
- 作業開始時に `overview.md` が存在するか確認する。
- `overview.md` がない場合は、作成するかをユーザーに確認する。
- 作成する場合は、rulesync と uv の導入状況を確認し、未導入なら導入案を提示して明示了承を得る。
- 了承後に導入・初期化・rulesync 生成を行い、保存先と確認結果を報告する。
- 導入後に、どのようなプロジェクトを行うのかをユーザーに確認し、`overview.md` の初期内容へ反映する。

禁止:

- 既存プロジェクトの README を dna_kernel の説明で置き換えない。
- `overview.md` がない状態で、プロジェクト種別や目的を推測して書き始めない。
- rulesync や uv の導入を、ユーザーの了承なしに自動実行しない。

参照:

- 導入手順: `skills/project-onboarding/SKILL.md`

## 既存プロジェクト注入モード

定義:
すでに README・docs・tools・設定を持つプロジェクトへ、既存構成を壊さず dna_kernel を追加する作業モード。

必須:

- 既存ファイルの有無と役割を確認し、変更予定を提示して了承を得る。
- ユーザーが注入先ディレクトリを明示した場合、そのディレクトリを注入先ルートとして扱う。
- `.rulesync/`, `rulesync.jsonc`, `docs/ja/dna-kernel/`, `docs/en/dna-kernel/`, `tools/kernel/` を追加・統合の候補として扱う。
- 既存 README はプロジェクト自身の入口として尊重し、必要な追記も確認後に行う。
- rulesync 生成物は `.gitignore` に追加する。

禁止:

- 明示された注入先ディレクトリから、monorepo ルートや Git ルートへ自動的に広げない。
- 既存 README、既存 docs、既存 tools を確認なしに移動・削除・上書きしない。
- dna_kernel の都合でプロジェクト固有の説明や導線を奪わない。

## DNA_KERNEL 開発モード

定義:
dna_kernel 自体を開発・修正する作業モード。このモードでは、利用先プロジェクトの概要確認や導入質問を省略する。

必須:

- ユーザーが dna_kernel 本体のルール・スキル・ツール・README の改善を依頼している場合、このモードとして扱う。
- 既存ファイルを読み、必要な変更を直接行う。
- 変更後は rulesync の dry-run または check など、影響範囲に応じた確認を行う。

禁止:

- dna_kernel 本体の修正依頼に対して、`overview.md` 作成や新規プロジェクト種別の質問を挟まない。

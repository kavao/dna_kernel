...
- [ ] v15.0.1 の 5 資産、URL、SHA-256、キャッシュ配置を設定ファイルへ定義する。
- [ ] `platform.system()` / `platform.machine()` の実値を OS・CPU キーへ正規化する。
- [ ] Unix の実行権限、Windows の `.exe`、未対応環境のエラー文を仕様化する。
- [ ] バイナリを Git 管理せず、初回取得時だけリポジトリ外キャッシュへ置くことを明記する。

### 2. Python 取得・実行ラッパーの実装

- [ ] 参照実装の取得・SHA-256・`--version` 検証・一時ファイル置換を移植する。
- [ ] OS・CPU 別設定の選択と、対応外環境の安全な失敗を実装する。
- [ ] 実行ラッパーが全引数と終了コードを透過することを実装する。
- [ ] 取得済みキャッシュの再検証を毎回行い、壊れたキャッシュを再取得できるようにする。
- [ ] 参照実装の `agentsmd` 専用通知フィルタは、現行 target に `agentsmd` がないため持ち込まず、必要になった時点で別途判断する。

### 3. テストの追加

- [ ] 対応する 5 組み合わせが正しい資産へ解決されることをテストする。
- [ ] `x86_64` / `amd64` / `aarch64` / `arm64` などの代表的な CPU 名を正規化することをテストする。
- [ ] 未対応 OS・CPU ではダウンロードせず、終了コード付きで案内することをテストする。
- [ ] SHA-256 不一致、`--version` 不一致、キャッシュ再利用・`--force` の挙動をネットワークなしでテストする。
- [ ] Python の通常テストを実行し、終了コード 0 を確認する。

### 4. コマンド・ドキュメントの移行

- [ ] 既存の `corepack pnpm dlx rulesync` 参照を、初回取得・dry-run・generate・check の新手順へ更新する。
- [ ] `uv run python tools/kernel/user_prefs.py sync` を generate 後に実行する現行ルールを維持する。
- [x] `delete: false` を採用し、生成物の自動削除を別作業に分離する方針と、生成物を直接編集しない原則を反映する。
- [ ] 日本語正本を先に更新し、英語 docs を同じ相対パスへ同期する。
- [ ] 参照ブランチの `sync_rules.py` は、現行リポジトリに旧コマンド利用箇所がないため初期導入では追加しない。互換入口が必要になった場合のみ追加する。

### 5. v15.0.1 生成結果の検証

- [ ] 現在の `rulesync.jsonc`（`claudecode`、`cursor`、`codexcli`、`rules`、`skills`）を v15.0.1 で dry-run する。
- [ ] 既存の 16.13.0 生成物との差分を一時ディレクトリで比較し、意図しない target・feature 欠落を確認する。
- [ ] v15.0.1 で `generate` と `generate --check` を実行する。
- [ ] generate 後に user-locale sync を行い、`.claude/` と `.cursor/` の副本が期待どおり残ることを確認する。
- [ ] 実装・テスト・文書・生成確認の結果を査証ログへ追記する。

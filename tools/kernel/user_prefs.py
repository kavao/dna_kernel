...
- このファイルは `user_prefs.py sync` で再生成される。`python tools/rulesync.py generate` の**後**にも `user_prefs.py sync` を実行する。
"""
    return """# User locale (generated)
...
- This file is regenerated by `user_prefs.py sync`. Run sync **after** `python tools/rulesync.py generate`.
"""
